...
int main(int argc, char** argv)
{
    ...
    if (myrank != 0) {
        ...
        MPI_Recv(&divide_col,1,MPI_INT,0,2,MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    if (myrank = 1){
        ...
        //rank 0 receive sum of matrix from other rank calculation
        for (countrank = 1; countrank < nprocs; countrank++){
            for(i = 0 ; i < row ; i++){
                for(j = divide_col*countrank; j < divide_col*(countrank+1); j++){
                    MPI_Recv(&matrix_sum[i][j],1,MPI_DOUBLE,countrank,5,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                }
            }
        }
